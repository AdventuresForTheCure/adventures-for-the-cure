...
angular
  .module('app')
  .factory('authorizationService', ['$http', '$q', 'identityService', 'Member', authorizationService]);
function authorizationService($http, $q, identityService, Member) {
  return {
    ...
    authorizeAuthenticatedMemberForRoute: function ($route) {
      if (
        ...
        ($route.current.params.id === identityService.currentMember._id || identityService.isAuthorized('admin'))
      ) {
        return true;
      } else {
        return $q.reject('not authorized');
  ...
  function checkForActiveMemberAndBioAndPic() {
    if (identityService.currentMember.isActive === false) {
      ...
      !identityService.currentMember.bio ||
      identityService.currentMember.bio.length <= 0 ||
      !identityService.currentMember.imgPath ||
      identityService.currentMember.imgPath.length <= 0
    ) {
...
function inventoryService($q, $http, $upload, InventoryItem) {
  return {
    ...
    delete: function (inventoryItem) {
      var dfd = $q.defer();
      $http
        .delete('/api/inventoryItems/' + inventoryItem._id)
        .success(function (data, status, headers, config) {
          dfd.resolve();
        })
        .error(function (error, status, headers, config) {
  ...
  memberService.getActiveMembers().then(function (members) {
    ...
    for (var i = 0; i < members.length; i++) {
      if (members[i].name === urlMemberName) {
        ...
        urlMemberName === '' &&
        identityService.isAuthenticated() &&
        identityService.currentMember.name === members[i].name
      ) {
        $scope.selectMember(members[i]);
